...
def image_reconstruction_kld_loss(mpm_logits, target_raw_patch_features,
                              masked_image_token_num, patch_feature_size, 
                              name="image_reconstruction_kld_loss"):
    ...
    image_loss = tf.reduce_mean(image_loss)
    tf.summary.scalar(name, image_loss)

    return image_loss

def image_reconstruction_kld_loss_with_mask(mpm_logits, target_raw_patch_features,
                              masked_image_token_num, patch_feature_size, 
                              mask=None,
                              name="image_reconstruction_kld_loss"):
    ...
    image_loss = tf.keras.losses.KLD(image_target_probs, image_pred_probs)
    if mask is not None:
        image_loss = tf.reduce_mean(image_loss * mask)
    else:
        image_loss = tf.reduce_mean(image_loss)
    tf.summary.scalar(name, image_loss)
