...
def get_pretrained_model(pretrain_model_name_or_path, **kwargs):
    if "/" not in pretrain_model_name_or_path:
        ...
        elif model_type == "fashionbert2":
            from .modeling_fashionbert2 import FashionBERT2PreTrainedModel
            return FashionBERT2PreTrainedModel.get(pretrain_model_name_or_path, **kwargs)
        elif model_type == "kaleidobert":
            from .modeling_kaleidobert import KaleidoBERTPreTrainedModel
            return KaleidoBERTPreTrainedModel.get(pretrain_model_name_or_path, **kwargs)
        else:
...
def get_config_path(model_type, pretrain_model_name_or_path):
    if model_type == 'bert':
        ...
        from .modeling_fashionbert2 import FashionBERT2PreTrainedModel
        config_path = FashionBERT2PreTrainedModel.pretrained_config_archive_map[
                    pretrain_model_name_or_path]
    elif model_type == "kaleidobert":
        from .modeling_kaleidobert import KaleidoBERTPreTrainedModel
        config_path = KaleidoBERTPreTrainedModel.pretrained_config_archive_map[
                    pretrain_model_name_or_path]
    else:
        raise ValueError("model_type should be in bert, roberta, albert, imagebert, videobert")
